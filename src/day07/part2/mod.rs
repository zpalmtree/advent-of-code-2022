...
pub fn solution(data: Vec<String>) -> String {
    ...
    let (directory_sizes, root_size) = calculate_directory_sizes(&file_system, 0);

    let space_free = disk_size - root_size;
    let space_needed = 30_000_000 - space_free;

    println!("There is currently {} bytes available to the system.", space_free);

    let mut smallest_candidate = root_size;

    for (_, directory_size) in directory_sizes {
        if directory_size < smallest_candidate && directory_size > space_needed {
            smallest_candidate = directory_size;
        }
    }

    return smallest_candidate.to_string();
}
